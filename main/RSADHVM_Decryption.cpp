...
void __fastcall TForm_Main::FormCreate(TObject *Sender)
{
	...
	//Max length input
	Edit_sK->MaxLength = 10;
}
//---------------------------------------------------------------------------

void __fastcall TForm_Main::CheckEdits()
{
	SpeedButton_Decrypt->Enabled = !Edit_enc_sK->Text.IsEmpty() && !Edit_VigenereKey->Text.IsEmpty() && !Edit_d->Text.IsEmpty() && !Edit_n->Text.IsEmpty();
	SpeedButton_SaveKey->Enabled = !Edit_enc_sK->Text.IsEmpty() && !Edit_VigenereKey->Text.IsEmpty() && !Edit_d->Text.IsEmpty() && !Edit_n->Text.IsEmpty();
}
//---------------------------------------------------------------------------

void __fastcall TForm_Main::Edit_enc_sKKeyPress(TObject *Sender, System::WideChar &Key)
{
...
void __fastcall TForm_Main::Edit_dKeyPress(TObject *Sender, System::WideChar &Key)
{
	if (!isdigit(Key) && Key != '\b') // Allow digits and backspace
	{
		Key = 0; // Discard the key press
...
void __fastcall TForm_Main::Edit_nKeyPress(TObject *Sender, System::WideChar &Key)
{
	if (!isdigit(Key) && Key != '\b') // Allow digits and backspace
	{
		Key = 0; // Discard the key press
...
void __fastcall TForm_Main::Edit_enc_sKChange(TObject *Sender)
{
    CheckEdits();
}
//---------------------------------------------------------------------------
...
void __fastcall TForm_Main::Edit_dChange(TObject *Sender)
{
    CheckEdits();
}
//---------------------------------------------------------------------------

void __fastcall TForm_Main::Edit_nChange(TObject *Sender)
{
	CheckEdits();
}
//---------------------------------------------------------------------------
...
void __fastcall TForm_Main::SpeedButton_SaveKeyClick(TObject *Sender)
{
	...
	if (SaveDialog_InputKey->Execute()) {
		...
		if (keyFile.is_open()) {
			...
			keyFile << "Encrypted sK = " << AnsiString(Edit_enc_sK->Text.c_str()) << endl;
			keyFile << "d = " << AnsiString(Edit_d->Text.c_str()) << endl;
			keyFile << "n = " << AnsiString(Edit_n->Text.c_str()) << endl;
...
void __fastcall TForm_Main::SpeedButton_MenuDecryptionClick(TObject *Sender)
{
	if (GroupBox_Decryption->Visible == false && GroupBox_HowToUse->Visible == true) {
...
{
	if (GroupBox_Decryption->Visible == true && GroupBox_HowToUse->Visible == false) {
...
void __fastcall TForm_Main::SpeedButton_ResetKeyClick(TObject *Sender)
{
	...
	Edit_sK->ReadOnly = false;
	Edit_VigenereKey->ReadOnly = false;
	Edit_d->ReadOnly = false;
	Edit_n->ReadOnly = false;
...
void __fastcall TForm_Main::SpeedButton_CalculationClick(TObject *Sender)
{
	...
	if (SaveDialog_Calculation->Execute()) {
		...
		if (keyFile.is_open()) {
			...
			keyFile << "Inv(sK) = " << AnsiString(Edit_InvsK->Text.c_str()) << endl;
			keyFile << "Vk = " << endl;
...
void __fastcall TForm_Main::SpeedButton_DecryptClick(TObject *Sender)
{
    //Validation
	if (Edit_enc_sK->Text.IsEmpty() || Edit_VigenereKey->Text.IsEmpty() || Edit_d->Text.IsEmpty() || Edit_n->Text.IsEmpty()) {
			ShowMessage("Please fill in all input key fields!");
			return;
	}

	//Read only input
	Edit_enc_sK->ReadOnly = true;
	Edit_VigenereKey->ReadOnly = true;
	Edit_d->ReadOnly = true;
	Edit_n->ReadOnly = true;

	//Get the input key parameter
	__int64 sK = StrToInt64(Edit_enc_sK->Text);
	String Vs = Edit_VigenereKey->Text;
	__int64 d = StrToInt64(Edit_d->Text);
	__int64 n = StrToInt64(Edit_n->Text);

	//Decrypting sK
	sK = ModPow(sK, d, n);
    Edit_sK->Text = IntToStr(sK);

	//Validating sK
	if (sK == 0) {
		ShowMessage("Error: sK is zero. Division by zero avoided.");
	...
	if (ClientDataSet1->RecordCount > 0)
	{
		try {
			...
			//Calculating inverse of sK in mod 256
			__int64 InvsK = ModInverse(sK, 256);
			Edit_InvsK->Text = IntToStr(InvsK);
			...
			while (!ClientDataSet1->Eof) {
				...
				int encrypted_r = (InvsK * (r - VkValue)) % 256;
				int encrypted_g = (InvsK * (g - VkValue)) % 256;
				int encrypted_b = (InvsK * (b - VkValue)) % 256;

				//Make sure encrypted value between 0-255
				encrypted_r = (encrypted_r + 256) % 256;
				encrypted_g = (encrypted_g + 256) % 256;
				encrypted_b = (encrypted_b + 256) % 256;
			...
			SaveCSVDialog->FileName = "rgb_data_decrypted.csv"; //Default file name
			...
			if (SaveCSVDialog->Execute()) {
				...
				if (!outFile.is_open()) {
					...
					try {
						...
						ShowMessage("Decrypted RGB data successfully exported to:\n" + CsvFileName);
			...
			TBitmap *decryptedImg = new TBitmap();
			decryptedImg->Width = Image_Width;
			decryptedImg->Height = Image_Height;
			...
			while (!ClientDataSet2->Eof) {
				...
				decryptedImg->Canvas->Pixels[x][y] = pixelColor;
				ClientDataSet2->Next();
			}

			//Save as BMP to Image_OutputSaveBMP (hidden)
			Image_OutputSave->Picture->Assign(decryptedImg);
			...
			SpeedButton_Decrypt->Enabled = false;
			ShowMessage("Image decrypted");
		}
		catch (Exception &E) {
			ShowMessage("Error: " + E.Message);
		}
